...
def validate_images(predict, model, generator_fn, save_path):
    x_s_batch, y_batch = next(generator_fn)
    x_batch = x_s_batch[0]
    s_batch = x_s_batch[1]
    for i in range(len(y_batch)):
        ...
        y = y_batch[i]
        if predict:
            y = model.predict([np.expand_dims(x, axis=0), np.expand_dims(s, axis=0)])
